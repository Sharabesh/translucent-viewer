...
class MRI {
    ...
    init = () => {
      var pr = new Promise( (resolve, reject) => {
          this.loadScript(this.struct_url, function() {return window.Struct != undefined;})
          .then(()  => {return this.loadScript(this.pako_url, function() {return window.pako != undefined;});})
          /*
          var pr=this.loadScript('https://cdn.rawgit.com/r03ert0/mrijs/v0.0.2/mri.js',function(){return window.MRI!=undefined});
          */
          .then(function() {
              if (!this.NiiHdrLE) {
                this.NiiHdrLE = Struct()
    ...
    createNifti = (dim, pixdim, v2w, data) => {
        let sizeof_hdr=348;
        ...
        this.NiiHdrLE.allocate();
        let niihdr = this.NiiHdrLE.buffer();
        let i;
        for(i in newHdr) {
            this.NiiHdrLE.fields[i] = newHdr[i];
        }
    ...
    loadMRIFromPath = (path, updateProgress) => {
        var pr = new Promise(function(resolve, reject) {
            ...
            req.onload = function(oEvent) {
                ...
                this.parseNifti(nii);
                this.mriPath = path;
                this.computeS2VTransform();
                this.MatrixVox2Mm = this.vox2mm();
                this.MatrixMm2Vox = this.mm2vox();
                console.log('done');
                resolve();
            };
    ...
    parseNifti = (nii) => {
      ...
      this.NiiHdrLE._setBuff(toBuffer(nii));
      var h = JSON.parse(JSON.stringify(this.NiiHdrLE.fields));
      if (h.sizeof_hdr != 348) {
        this.NiiHdrBE._setBuff(toBuffer(nii));
        h = JSON.parse(JSON.stringify(this.NiiHdrBE.fields));
        endianness = 'be';
      }
      ...
      this.hdr = nii.slice(0, vox_offset);
      this.datatype = h.datatype;
      this.dim = [h.dim[1],h.dim[2],h.dim[3]];
      this.datadim = h.dim[4];
      console.log(this.datadim);
      this.pixdim = [h.pixdim[1],h.pixdim[2],h.pixdim[3]];

      switch (this.datatype) {
      case 2: // UCHAR
        this.data = new Uint8Array(nii,vox_offset);
      break;
      case 256: // INT8
        this.data = new Uint8Array(nii,vox_offset);
      break;
      case 4: // SHORT
        if (endianness == 'le')
            this.data = new Int16Array(nii,vox_offset);
        else
            this.data = this.swapInt16(new Int16Array(nii,vox_offset));
      break;
      case 8:  // INT
        if (endianness == 'le')
            this.data = new Int32Array(nii,vox_offset);
        else
            this.data = this.swapInt32(new Int32Array(nii,vox_offset));
      break;
      case 16: // FLOAT
        if (endianness == 'le')
            this.data = new Float32Array(nii,vox_offset);
        else
            this.data = this.swapFloat32(new Float32Array(nii,vox_offset));
      break;
      case 256: // INT8
        this.data = new Int8Array(nii,vox_offset);
      break;
      case 512: // UINT16
        if (endianness == 'le')
            this.data = new Uint16Array(nii,vox_offset);
        else
            this.data = this.swapUint16(new Uint16Array(nii,vox_offset));
      break;
      default:
        console.error('Unknown dataType: ' + this.datatype);
        }
    };
    ...
    */
    mm2vox = () => {
        var m = this.vox2mm();
        var im = this.inv4x4Mat([...m[0], ...m[1], ...m[2], ...m[3]]);
    ...
    */
    computeS2VTransform = () => {
        ...
        this.s2v = {
            ...
            X: (mi.v > 0) ? 0 : (this.dim[0] - 1), // starting value for space coordinate x when voxel coordinate i starts
            Y: (mj.v > 0) ? 0 : (this.dim[1] - 1), // same for y
            Z: (mk.v > 0) ? 0 : (this.dim[2] - 1), // same for z
            sdim: [],
            wpixdim: []
        };
        this.s2v.sdim[mi.i] = this.dim[0];
        this.s2v.sdim[mj.i] = this.dim[1];
        this.s2v.sdim[mk.i] = this.dim[2];
        this.s2v.wpixdim[mi.i] = this.pixdim[0];
        this.s2v.wpixdim[mj.i] = this.pixdim[1];
        this.s2v.wpixdim[mk.i] = this.pixdim[2];
    }
}